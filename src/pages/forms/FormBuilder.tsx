...
import { FormConfig, InputElement, ButtonElement, FormValues } from '../../types/form';
...
const FormBuilder: React.FC = () => {
	...
	const handleAddElement = (element: InputElement | ButtonElement) => {
		const newOrder =
			formConfig.elements.length > 0 ? Math.max(...formConfig.elements.map((el) => el.order)) + 1 : 0;
	...
	const handleUpdateElement = (updatedElement: InputElement | ButtonElement) => {
		setFormConfig({
			...formConfig,
			elements: formConfig.elements.map((el) => (el.id === updatedElement.id ? updatedElement : el))
		});
	};
	...
	const handleReorderElements = (elements: (InputElement | ButtonElement)[]) => {
		setFormConfig({
			...formConfig,
			elements
		});
