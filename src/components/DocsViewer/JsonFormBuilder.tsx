...
import { Edit3, Save, Code, Plus, Trash2, ChevronUp, ChevronDown } from 'lucide-react';
...
const JsonFormBuilder: React.FC<JsonFormBuilderProps> = ({ formData, onFormUpdate, placeholderNames }) => {
		...
		try {
			...
			parsed.fields.forEach((field: any, index: number) => {
				if (!field.id || !field.type || !field.label) {
					throw new Error(`Field at index ${index} missing required properties (id, type, label)`);
				}
			});
	...
	return (
		...
		>
			...
			{isEditing ? (
				...
				<div className="p-4">
					<div className="mb-4">
						<label className="block text-sm font-medium text-neutral-700 mb-1">Form Title</label>
						<input
							type="text"
							value={formData.title}
							onChange={(e) => onFormUpdate({ ...formData, title: e.target.value })}
									...
									>
										<div className="flex justify-between mb-3">
											<div className="flex items-center gap-2">
												...
												<div className="text-sm text-neutral-500">({field.type})</div>
											</div>
